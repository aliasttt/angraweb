# Generated migration: add blog article "Web Sitesi Hızı / How to Improve Website Speed"
...
def add_blog_post(apps, schema_editor):
    ...
    if BlogPost.objects.filter(slug='web-sitesi-hizi-nasil-artirilir').exists():
        return
    BlogPost.objects.create(
        title='Web Sitesi Açılış Hızı Nasıl Artırılır? (SEO ve Performans Rehberi)',
        title_en='How to Improve Website Speed (SEO & Performance Guide)',
        slug='web-sitesi-hizi-nasil-artirilir',
        slug_en='how-to-improve-website-speed',
        category='Web Development',
        tags='web sitesi hızı, seo, website performance, site speed',
        excerpt='Web sitesi hızını artırmak SEO, kullanıcı deneyimi ve dönüşüm oranları için kritik öneme sahiptir. Bu rehberde site hızını artıran teknikleri öğrenin.',
        excerpt_en='Website speed is critical for SEO, user experience, and conversions. Learn the best techniques to improve website loading speed in this guide.',
...
def remove_blog_post(apps, schema_editor):
    BlogPost = apps.get_model('main', 'BlogPost')
    BlogPost.objects.filter(slug='web-sitesi-hizi-nasil-artirilir').delete()


class Migration(migrations.Migration):

    dependencies = [
        ('main', '0009_blog_category_slug_en_tags'),
    ]
